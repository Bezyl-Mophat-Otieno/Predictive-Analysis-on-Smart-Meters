...
def preprocess_data(input_csv, output_dir, file_name):
    ...
    keeping only the load from Sub_metering_3, handling missing values, and saving the preprocessed data.
    ...
    # Drop unnecessary columns including other sub-meterings
    data.drop(['Date', 'Time', 'Global_reactive_power', 'Sub_metering_1', 'Sub_metering_2'], axis=1, inplace=True)
    
    # Rename Sub_metering_3 to reflect its unit
    data.rename(columns={
        'Global_active_power': 'Power',
        'Voltage': 'Voltage',
        'Global_intensity': 'Current',
        'Sub_metering_3': 'Load(Wh)'  # Focus only on Sub_metering_3 load
    }, inplace=True)
